...
testClientReauthentication brig = do
    ...
    Just c <- decodeBody <$> (addClient brig uid payload1 <!! const 201 === statusCode)
    ...
    Just c2 <- decodeBody <$> (addClient brig uid2 payload1 <!! const 201 === statusCode)
    Just c3 <- decodeBody <$> (addClient brig uid2 payload2 <!! const 201 === statusCode)
    ...
    Just c4 <- decodeBody <$> addClient brig uid payload3
    deleteClient brig uid (clientId c4) Nothing !!! const 200 === statusCode
    Just c5 <- decodeBody <$> addClient brig uid2 payload3
...
testListPrekeyIds brig = do
    ...
    Just c <- decodeBody <$> addClient brig uid new
...
testRemoveClient brig cannon = do
    ...
    Just c <- decodeBody <$> addClient brig uid (client PermanentClient (someLastPrekeys !! 10))
    -- Missing password
    deleteClient brig uid (clientId c) Nothing !!! const 403 === statusCode
...
testUpdateClient brig = do
    ...
    Just c <- decodeBody <$> addClient brig uid clt
    get (brig . paths ["users", toByteString' uid, "prekeys", toByteString' (clientId c)]) !!! do
        const 200 === statusCode
        const (Just $ ClientPrekey (clientId c) (somePrekeys !! 0)) === decodeBody
...
testPreKeyRace brig = do
    ...
    Just c <- decodeBody <$> addClient brig uid (defNewClient PermanentClient pks (someLastPrekeys !! 0))
